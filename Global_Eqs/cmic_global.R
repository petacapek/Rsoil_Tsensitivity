cmic_global<-function(data){
  ...
  estim<-function(fr){
    ...
    est<-function(x){
      ...
      Ctot<-as.numeric(fr[, "Biomass"])

      yhat<-par[1]+Ctot*par[2]
    ...
    par_mcmc<-modMCMC(f=est, p=c(k=1, Eas=30),
                      lower=c(k=1e-60, Eas=3e-6),
                      upper=c(k=1e50, Eas=100000), niter=50000)
