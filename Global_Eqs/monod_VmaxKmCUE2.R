monod_VmaxKmCUE2<-function(data){
  ...
  estim<-function(fr){
    ...
    est<-function(x){
      ...
      yhat<-x[1]*exp(x[3]/8.314/Temp)*Ctot*Cmic/(x[2]*exp(x[4]/8.314/Temp)+Ctot)*(1-x[5]-x[6]*(Temp-273.15))
    ...
    par_mcmc<-modMCMC(f=est, p=c(1, 30, -60000, -60000, 0.5, 0.015),
                      lower=c(1e-30, 3e-6, -300000, -300000, 0.005, 0.00000015),
                      upper=c(1e30, 30000, 30000, 30000, 1, 150000), niter=50000)
