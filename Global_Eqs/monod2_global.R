monod2_global<-function(data){
  ...
  estim<-function(fr){
    ...
    est<-function(x){
      ...
      CNs<-as.numeric(fr[, "CNs"])

      yhat<-x[1]*Ctot*Cmic/(x[2]+Ctot)*x[3]*((1/CNs)/(6/42))^x[4]
    ...
    par_mcmc<-modMCMC(f=est, p=c(1, 30, 0.43, 0.6),
                      lower=c(1e-30, 3e-6, 1e-6, 1e-6),
                      upper=c(1e30, 30000, 100000, 1), niter=50000)
